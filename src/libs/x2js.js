(function (a, b) {
  ...
  return function (z) {
    ...
    function x(B) {
      var C = B.localName;
      if (C == null) C = B.baseName;
      if (C == null || C == "") C = B.nodeName;
      return C;
    }
    function r(B) {
      return B.prefix;
    ...
    function w(C, F, D, E) {
      ...
      for (; B < C.length; B++) {
        var G = C[B];
        if (typeof G === "string") if (G == E) break;
        else {
          if (G instanceof RegExp) if (G.test(E)) break;
          else if (typeof G === "function") if (G(F, D, E)) break;
        }
      }
      return B != C.length;
    }
    function n(D, B, C) {
      switch (z.arrayAccessForm) {
        case "property":
          if (!(D[B] instanceof Array)) D[B + "_asArray"] = [D[B]];
          else D[B + "_asArray"] = D[B];
          break;
      }
      if (!(D[B] instanceof Array) && z.arrayAccessFormPaths.length > 0) {
        if (w(z.arrayAccessFormPaths, D, B, C)) D[B] = [D[B]];
      }
    }
    function a(G) {
      var E = G.split(/[-T:+Z]/g);
      ...
      if (D.length > 1) F.setMilliseconds(D[1]);
      if (E[6] && E[7]) {
    ...
    function q(D, B, C) {
      if (z.datetimeAccessFormPaths.length > 0) {
        var E = C.split(".#")[0];
        if (w(z.datetimeAccessFormPaths, D, B, E)) return a(D);
        else return D;
      } else return D;
    }
    function b(E, C, B, D) {
    ...
    function A(D, J) {
      if (D.nodeType == h.DOCUMENT_NODE) {
        ...
        if (D.nodeType == h.ELEMENT_NODE) {
          ...
          if (K["#text"] != null) {
            K.__text = K["#text"];
            if (K.__text instanceof Array) K.__text = K.__text.join("\n");
            if (z.stripWhitespaces) K.__text = K.__text.trim();
            delete K["#text"];
            if (z.arrayAccessForm == "property") delete K["#text_asArray"];
          ...
          if (K.__cnt == 0 && z.emptyNodeForm == "text") K = "";
          else {
            if (K.__cnt == 1 && K.__text != null) K = K.__text;
            else {
              if (K.__cnt == 1 && K.__cdata != null && !z.keepCData) {
                K = K.__cdata;
              } else {
                if (
                  K.__cnt > 1 && K.__text != null && z.skipEmptyTextNodesForObj
                ) {
                  if (
          ...
          if (
            D.nodeType == h.TEXT_NODE || D.nodeType == h.CDATA_SECTION_NODE
          ) return D.nodeValue;
        }
      }
    }
    function o(I, F, H, C) {
      ...
      if (H != null) {
        for (var G = 0; G < H.length; G++) {
          var D = H[G];
          var B = I[D];
          if (z.escapeMode) B = s(B);
          E += " " + D.substr(z.attributePrefix.length) + "=";
          if (z.useDoubleQuotes) E += '"' + B + '"';
          else E += "'" + B + "'";
        }
      }
      if (!C) E += ">";
    ...
    function y(C, B) {
      if (
        ...
        B.toString().indexOf("__") == 0 || (C[B] instanceof Function)
      ) return true;
      else return false;
    }
    function m(D) {
    ...
    function l(D, B, C) {
      return z.jsonPropertiesFilter.length == 0 || C == "" ||
    ...
    function g(C) {
      var B = "";
      if (C.__cdata != null) B += "<![CDATA[" + C.__cdata + "]]>";
      if (C.__text != null) {
        if (z.escapeMode) B += s(C.__text);
        else B += C.__text;
      }
      return B;
    }
    function d(C) {
      var B = "";
      if (C instanceof Object) B += g(C);
      else {
        if (C != null) {
          if (z.escapeMode) B += s(C);
          else B += C;
        }
      }
      return B;
    ...
    function e(I, H) {
      ...
      if (F > 0) {
        for (var E in I) {
          if (y(I, E) || (H != "" && !l(I, E, p(H, E)))) continue;
          var D = I[E];
          var G = c(D);
          if (D == null || D == undefined) B += o(D, E, G, true);
          else {
            if (D instanceof Object) {
              if (D instanceof Array) B += f(D, E, G, H);
              else {
    ...
    this.parseXmlString = function (D) {
      ...
      if (window.DOMParser) {
        ...
        if (!F) {
          try {
            B =
              G.parseFromString("INVALID", "text/xml").getElementsByTagName(
                "parsererror",
              )[0].namespaceURI;
          } catch (C) {
            B = null;
        ...
        try {
          ...
          if (
            B != null && E.getElementsByTagNameNS(B, "parsererror").length > 0
          ) E = null;
        } catch (C) {
          E = null;
        }
      } else {
        if (D.indexOf("<?") == 0) D = D.substr(D.indexOf("?>") + 2);
    ...
    this.toXmlDateTime = function (B) {
      if (B instanceof Date) return B.toISOString();
        ...
        if (typeof (B) === "number") return new Date(B).toISOString();
        else return null;
      }
    };
    ...
    this.xml_str2json = function (B) {
      ...
      if (C != null) return this.xml2json(C);
      else return null;
    };
